...
describe("UniclyXUnicVault", function () {
  ...
  describe("Staking rewards", function () {
    ...
    it.only("Should reward user with xUNIC", async () => { // TODO: Remove only

      // TODO: remove all console logs

      const user1StakeInfo = await uniclyXUnicVault.userInfo(uAPE_ETH_POOL_ID, addr1.address);
      ...
      const uAPELPTokenContract = new ethers.Contract(uAPELPToken, erc20Abi, addr1);
      const uAPELPTokenBalance = await uAPELPTokenContract.balanceOf(addr1.address);
      expect(uAPELPTokenBalance).to.be.gt(0);
      await uAPELPTokenContract.connect(addr1).approve(uniclyXUnicVault.address, ethers.utils.parseEther("10000"));
      await uniclyXUnicVault.connect(addr1).deposit(uAPE_ETH_POOL_ID, uAPELPTokenBalance);
      console.log("uAPELPTokenBalance", uAPELPTokenBalance.toString());
      const userStakeInfo = await uniclyXUnicVault.userInfo(uAPE_ETH_POOL_ID, addr1.address);
      expect(userStakeInfo.amount).to.equal(uAPELPTokenBalance);
      expect(await xUnicToken.balanceOf(addr1.address)).to.equal(0);
      let pendingxUNICs = await uniclyXUnicVault.pendingxUNICs(uAPE_ETH_POOL_ID, addr1.address);
      expect(pendingxUNICs).to.equal(0);
      const day = 60 * 60 * 24;
      await advanceTime(day);
      pendingxUNICs = await uniclyXUnicVault.pendingxUNICs(uAPE_ETH_POOL_ID, addr1.address);
      expect(pendingxUNICs).to.be.gt(0);
      const prevPendingxUNICs = pendingxUNICs;
      console.log(prevPendingxUNICs.toString());
      await uniclyXUnicVault.connect(addr1).deposit(uAPE_ETH_POOL_ID, 0);
      pendingxUNICs = await uniclyXUnicVault.pendingxUNICs(uAPE_ETH_POOL_ID, addr1.address);
      expect(pendingxUNICs).to.equal(0);
      console.log((await xUnicToken.balanceOf(addr1.address)).toString());
      console.log(prevPendingxUNICs.toString());
      expect(await xUnicToken.balanceOf(addr1.address)).to.equal(prevPendingxUNICs);
